...
// Auto-seed: chi insert neu DB chua co data nao (idempotent, an toan goi luc startup)
export async function seedIfEmpty(db) {
  const news = await db.execute('SELECT COUNT(*) as c FROM news');
  const acts = await db.execute('SELECT COUNT(*) as c FROM activities');
  if (Number(news.rows[0].c) > 0 || Number(acts.rows[0].c) > 0) return false;

  console.log('[seed] Empty database detected, inserting initial content...');
  await insertAll(db);
  console.log('[seed] Inserted', newsSeed.length, 'news +', activitiesSeed.length, 'activities');
  return true;
}

// Force seed: clear + reinsert (dung qua CLI khi muon refresh data)
export async function forceSeed(db) {
  console.log('[seed] Force seed: clearing news + activities...');
